...
const run = async (client, message, [action, key, ...value]) => {
    ...
    if (action === 'add') {
        ...
        if (!defaults[key])
            return message.reply('This key does not exist in the settings');
        const response = await client.functions.awaitReply(message.author.id, message.channel, `Are you sure you want to permanently delete ${key} from all guilds? This **CANNOT** be undone.`);
