...
export const run: RunFunction = async (client: Bot, message, args) => {
  ...
  if (!args[0]) {
    ...
    const myCommands = client.commands.filter(
      (cmd) => client.levelCache[cmd.conf.permLevel] <= message.author.level
    );
    ...
    const sorted = myCommands
      ...
      .sort((p, c) =>
        p.help.category > c.help.category
          ? 1
          : p.conf.name > c.conf.name && p.help.category === c.help.category
          ? 1
          : -1
      );
    sorted.forEach((c) => {
    ...
    if (client.commands.has(cmd)) {
      ...
      if (message.author.level < client.levelCache[command.conf.permLevel])
        return;
      message.channel.send(
        client.embed({
          title: "Command list",
